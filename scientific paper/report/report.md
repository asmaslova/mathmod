...
где $x \sim y$ означает, что $x$ и $y$ соединены ребром, 

$ X^i_{y \to x}(x) = (X^1(x), ..., X^{i-1}(x), X^i(y), X^{i+1}(x), ..., X^F(x)) $

и $ X^i_{y \to x} = X(z) \text{для всех} z \neq x $, и 

$ F(x,y) = \fraq{1}{F} \displaystyle\sum_{i=1}{F} \mathbb{1} \{ X^i(x) = X^i(y) \} $

это доля культурных особенностей, которые имеют общие вершины $x$ и $y$ [@lanchier:paper:bash].
