...
$Ω_{ax}  f(X) = \displaystyle\sum_{x \in V} \displaystyle\sum_{x \sim y} \displaystyle\sum_{i=1}^{F} \frac{1}{2F}  \big\[ \frac{F(x,y)}{1 - F(x,y)} \]  \mathbb{1}  \left\{ X^i(y) \neq X^i(y) \right\} \big\[ f(X^i_{y \to x}) \big\]$
